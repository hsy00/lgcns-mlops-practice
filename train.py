...
sys.excepthook = handle_exception  # exception 발생 시 에러 traceback 제공
...
if __name__ == "__main__":
    ...
    X.assign(rent=y).to_csv(
        ...
        os.path.join(DATA_PATH, "storage", "house_rent_train_featuress.csv"),
        index=False,
    )
    ...
    mlflow.set_tracking_uri("./mlruns")  # MLflow artifact 저장소 위치

    logger.debug("Using MLflow to track a experiment...")
    for i, params in enumerate(param_set):
        ...
        with mlflow.start_run(run_name=f"Run {i}"):
            ...
            )  # GradientBoostingRegressor로 tag를 위한 이름 생성
